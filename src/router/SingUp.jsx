...
export default function SignUp() {
  ...
  const validate = () => {
    const e = {};
    if (mode === "signup" && !values.name.trim()) e.name = "Name is required";
    if (!values.contact.trim()) e.contact = "Email or phone required";

    const emailOk = /\S+@\S+\.\S+/.test(values.contact);
    const phoneOk = /^\+?\d{7,15}$/.test(values.contact);

    if (!emailOk && !phoneOk) e.contact = "Enter valid email or phone";
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === "signup") {
      if (users.find((u) => u.contact === values.contact)) {
        setErrors({ contact: "Already registered" });
        return;
      }
      users.push(values);
      ...
      setMode("login");
      setValues({ name: "", contact: "", password: "" });
    } else {
      const found = users.find(
        (u) => u.contact === values.contact && u.password === values.password
      );
      if (!found) {
        alert("Invalid credentials");
        return;
      }
      alert(`Welcome back, ${found.name || "User"}!`);
    }
  };
    ...
    <div className="auth-page">
      <main className="auth-hero">
        ...
        <div className="form-wrap">
          <div className="card">
            <h1>
              {mode === "signup" ? "Create an account" : "Log in to Exclusive"}
            </h1>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {mode === "signup" && (
                ...
                >
                  <img src={googleLogo} alt="Google Play" style={{ height: 20, marginRight: 8 }} />
                  Sign up with Google
                </button>
              )}
